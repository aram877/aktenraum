...
Switch by editing `docker/.env` and running `docker compose up -d auto-tagger aktenraum-api` (restart alone does NOT re-read env files — must use `up -d`). After Python source changes also use `--build`.
