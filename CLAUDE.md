...
| nginx Docker build fails inside `pnpm install` on `canvas@*` with "Could not find any Python installation"   | `jscanify` (added in mobile-document-scan Phase 2a) declares `canvas` (Cairo/Pango native binding) + `jsdom` as hard runtime deps for its Node-side entry, even though we only import its `/client` subpath in the browser. Alpine's slim builder image has no Python/Cairo to compile `canvas`, so pnpm's postinstall fails. Fix: root `package.json` has `pnpm.neverBuiltDependencies: ["canvas"]`, which tells pnpm to install the JS files but skip the native compile. The `/client` subpath never touches the `canvas` module so this is safe. Do NOT swap to `--ignore-scripts` (would also break `esbuild`'s prebuilt-binary postinstall). |
...
| Mobile document scan (`/scan`)                                                                  | ✅ Phase 2a — `<input type="file" accept="image/*" capture="environment">` invokes the OS camera; thumbnail grid with ↑↓ reorder / rotate / "Ecken anpassen" / delete; filename input defaults `scan-YYYY-MM-DD-HHmmss`; **client-side PDF composition via `pdf-lib`** (decode → rotate+crop on canvas → JPEG 0.85 → embed in A4-portrait page with letterboxing, downscale longest side to 3200 px); **auto edge-detection + 4-corner perspective warp** via `jscanify` + `@techstark/opencv-js` opens automatically after every capture — user nudges corners (SVG polygon overlay + 4 PointerEvent drag handles) and taps Übernehmen to apply the warp, OR Überspringen to keep the raw photo. The OpenCV.js runtime (`scan-engine-<hash>.js` chunk, ~3.5 MB gzip) lives in a Vite manualChunk that loads ONLY when `/scan` opens AND the engine is invoked. Main `index-<hash>.js` bundle UNCHANGED at 116 KB gzip; `Scan-<hash>.js` route chunk 186 KB gzip (pdf-lib). Uploads through the existing `/api/documents/upload` endpoint with **zero backend changes**. MAX_PAGES=30. Phase 2b (live `getUserMedia` preview with edge overlay + auto-shutter) planned in `openspec/changes/mobile-document-scan/` task group 6b; ships only if rapid-fire capture turns out to be valuable on top of the post-capture corner-adjuster flow. |
